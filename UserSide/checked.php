...
<head>
  ...
  <style type="text/css">
   .animateuse{
     animation: leelaanimate 3s infinite;
   }
...
<body>
 <div class="container text-center" >
  <br><br>
  <h1 class="text-center text-success text-uppercase animateuse" > Quiz result</h1>
  <br><br><br><br>
             ...
             if(!empty($_POST['optionsRadios'])) 
                  {
                      ...
                      $checked_count = count($_POST['optionsRadios']);
                      // print_r($_POST);
                      $selected = $_POST['optionsRadios'];
                     // print_r($selected);

                      $sql = "select * from que_master where que_id IN ($list)";
                     // echo $sql. "<br>" ;
                      $result= mysqli_query($con,$sql);
                      
                       $row1 = mysqli_num_rows($result);
                       //xprintf("no of rows are" .$row1);
                      $i = 1;
                          while($row=mysqli_fetch_assoc($result))
                              {
                                   $flag = $row['answer'] ;
                                   //echo $flag."<br><br><br>";
                                   $flag = $row['answer'] == @$selected[$i];
                                   //print_r($selected[$i]);
...
<td>
  <?php
  echo "Out of ".$row1.", You have attempt ".$checked_count." questions."; ?>
</td>
<tr>
  <td>Your no of correct answer is :</td>
